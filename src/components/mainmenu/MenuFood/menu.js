...
class Menu extends Component{
    ...
    getListFoodMessage() {
        ...
        if (mylistFood.error) return 'Không lấy được dữ liệu do kết nối mạng có vấn đề!';    
        return true;
    }
    componentDidMount(){
    ...
    render(){
        return(
           <View style={{ backgroundColor: '#f1f8ff', flex: 1 }}>
               {this.getListFoodMessage() == true ? null : <Text style={{ color: 'red', fontSize: 20, fontWeight: 'bold' }}>{this.getListFoodMessage()}</Text>}
               <FlatList
...
export default connect(mapStateToProps, listFoodAction)(Menu);