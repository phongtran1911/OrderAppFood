...
class Revenue1Month extends Component {
  constructor(props) {
    super(props);
    this.state = {
      morningMoney: 0,
  ...
  async componentDidMount() {
    const moneyMorning = await getMorningRevenueInMonth()
      .then(result => {
        return result[0].Total;
      })
      .catch(err => console.log(err));
    const moneyAfternoon = await getAfternoonRevenueInMonth()
      .then(result => {
        return result[0].Total;
      })
      .catch(err => console.log(err));
      ...
      <View style={styles.container}>
        <View style={wrapper}>
          <View style={styles.serviceTitle}>
            <Text style={styles.textStyle}>Doanh thu buổi sáng của tháng</Text>
        ...
        <View style={wrapper}>
          <View style={styles.serviceTitle}>
            <Text style={styles.textStyle}>Doanh thu buổi chiều của tháng</Text>
