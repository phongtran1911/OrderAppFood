...
class ListFoodAdd extends Component {
    ...
    listCheckFoodAddUse(result) {
        ...
        this.setState({}, () => saver.setDataFoodTypeID(resultID, resultName));
    }
    render() {
        return (
            <View style={{ flex: 1 }}>
                <FlatList
                    ...
                    renderItem={({ item }) =>
                        <View style={{ flexDirection: 'row', width: "33.3%" }}>
                            ...
                            />
                            <Text style={{ alignSelf: 'center', textAlign: 'center' }} onPress={() => this.onCheckedFoodAdd(item.id)}>{item.Name}</Text>
                        </View>
                    }
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) =>
                            <View style={{ flexDirection: 'row', width: "33.3%" }}>
                                ...
                                />
                                <Text style={{ alignSelf: 'center', textAlign: 'center' }} onPress={() => this.onCheckedFoodAddUse(item.id)}>{item.Name}</Text>
                            </View>
                        }
                        keyExtractor={this._keyExtractor}
                    />
...
export default connect(mapStateToProps, { fetchDataFoodAdd, fetchDataFoodAddUse })(ListFoodAdd);